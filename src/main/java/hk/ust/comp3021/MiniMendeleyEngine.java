...
public class MiniMendeleyEngine {
  ...
  /**
   * Create a new user and add the user to the field `users` You may need to
   * remove the `return null` from the skeleton.
   *
  ...
  /**
   * Add a new comment with a given AddCommentAction. You need to: (1) Create the
   * corresponding kind of the comment object. The comment id is "Comment_x",
   * where x is equal to the number of comments in the field `comments`. (2) Add
   * the comment object to the field `comments` (3) Update the comment list in the
   * commented paper or comment (4) Update the comment list in the current user.
   * (5) If the comment is added successfully, please set action result to be true
   * in `action`. Otherwise, please set action result to be false and the return
   * value to be null. (6) No matter the action succeeds or not, you need to add
   * the action to `actions`. You may need to remove the `return null` from the
   * skeleton.
   *
  ...
  public Comment processAddCommentAction(User curUser, AddCommentAction action) {
    ...
    try {
      Comment comment = new Comment("Comment_" + comments.size(), new Date(), action.getCommentStr(), curUser,
  ...
  /**
   * Add a label to a paper with a given AddLabelAction. You need to: (1) Create a
   * new label. The label id is "Label_x", where x is the number of labels in the
   * field `labels`. Add the label to the field `labels`. (2) Update the label
   * list of the paper (3) Update the label list of the current user. (4) If the
   * label is added successfully, please set action result to be true in `action`.
   * Otherwise, please set action result to be false and the return value to be
   * null. (5) No matter the action succeeds or not, you need to add the action to
   * `actions`. You may need to remove the `return null` from the skeleton.
   *
   * @param curUser: Current user
  ...
  /**
   * Download the specified papers to a bib file with a given DownloadPaperAction
   * action. You need to: (1) Create a bibfile according to `action` (2) Dump the
   * paper info to the bibfile (3) If the papers are exported successfully, please
   * set action result to be true in `action`. Otherwise, please set action result
   * to be false. (4) No matter the action succeeds or not, you need to add the
   * action to `actions`.
   *
   * @param curUser: Current user
  ...
  /**
   * Upload the papers from a bib file to `paperBase`. You need to (1) Add all the
   * papers to the field `paperBase`. (2) Add all the authors to the field
   * `researchers` (3) Update the paper list of the researchers by adding the
   * uploaded papers. If the researcher does not exist in `researchers`, which is
   * identified according to his or her name, you need to create a new research
   * object, of which the id is "Researcher_x", x is the number of the researchers
   * in `researchers`. (4) If the papers are uploaded successfully, please set
   * action result to be true in `action`. Otherwise, please set action result to
   * be false. (5) No matter the action succeeds or not, you need to add the
   * action to `actions`.
   *
   * @param curUser: Current user
  ...
  /**
   * Search the papers with a given SearchPaperAction action. You need to support
   * four kinds of search mode. No matter the action succeeds or not, you need to
   * add the action to `actions`. You may need to remove the `return null` from
   * the skeleton.
   *
   * @param curUser: Current user
   * @param action:  SearchPaperAction actioin
   * @return: The searched papers. If not paper found, please return an empty
   *          ArrayList.
   */
